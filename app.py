...
# ── MAIN DASHBOARD ────────────────────────────────────
    ...
    if page == "🏠 Overview":
        ...
        col1.metric("Total Students",
                    f"{len(filtered_df):,}")
        col2.metric("Total Enrolled",
                    f"{int(filtered_df['Converted'].sum()):,}")
        col3.metric("Conversion Rate",
                    f"{filtered_df['Converted'].mean()*100:.1f}%")
        col4.metric("Total Revenue",
                    f"₹{filtered_df['Revenue'].sum()/10000000:.1f} Cr")

        st.divider()
        col5, col6 = st.columns(2)
        col5.metric("Avg Final Price",
                    f"₹{filtered_df['Final_Price'].mean():,.0f}")
        col6.metric("Avg Discount Given",
                    f"{filtered_df['Discount_%'].mean():.1f}%")
    ...
    # ── PAGE 2: PROGRAM ANALYSIS ──────────────────────
        ...
        with col1:
            prog_rev = filtered_df.groupby('Program_Type')['Revenue'].sum().reset_index()
            prog_rev['Revenue_Cr'] = (prog_rev['Revenue']/10000000).round(2)
        ...
        with col2:
            prog_conv = filtered_df.groupby('Program_Type')['Converted'].mean().reset_index()
            prog_conv['Conversion_%'] = (prog_conv['Converted']*100).round(1)
        ...
        # Program wise avg price
        prog_price = filtered_df.groupby('Program_Type')['Final_Price'].mean().reset_index()
    ...
    # ── PAGE 3: DISCOUNT ANALYSIS ─────────────────────
        ...
        disc_conv['Conversion_Rate'] = (disc_conv['Conversion_Rate']*100).round(1)
        ...
        st.info("💡 Key Insight: 20% discount is the sweet spot. "
                "Below 20% — conversion is low (~48%). "
                "At 20% — conversion jumps to 70%. "
                "Above 20% — no further improvement in conversion!")
    ...
    # ── PAGE 4: COLLEGE TIER ──────────────────────────
        ...
        with col1:
            tier_conv = filtered_df.groupby('College_Tier')['Converted'].mean().reset_index()
            tier_conv['Conversion_%'] = (tier_conv['Converted']*100).round(1)
        ...
        with col2:
            tier_rev = filtered_df.groupby('College_Tier')['Revenue'].sum().reset_index()
            tier_rev['Revenue_Cr'] = (tier_rev['Revenue']/10000000).round(2)
        ...
        income_conv = filtered_df.groupby('Family_Income')['Converted'].mean().reset_index()
        income_conv['Conversion_%'] = (income_conv['Converted']*100).round(1)
